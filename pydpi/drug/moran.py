    ...
    **Internal used only**
    ...
    for i in mol.GetAtoms():
        temp = GetRelativeAtomicProperty(
            i.GetSymbol(), propertyname=propertylabel)
        prolist.append(temp)
...
def CalculateMoranAutoMass(mol):
    ...
    for i in range(8):
        res['MATSm' + str(i + 1)] = _CalculateMoranAutocorrelation(mol,
                                                                   lag=i + 1, propertylabel='m')
...
def CalculateMoranAutoVolume(mol):
    ...
    for i in range(8):
        res['MATSv' + str(i + 1)] = _CalculateMoranAutocorrelation(mol,
                                                                   lag=i + 1, propertylabel='V')
...
def CalculateMoranAutoElectronegativity(mol):
    ...
    for i in range(8):
        res['MATSe' + str(i + 1)] = _CalculateMoranAutocorrelation(mol,
                                                                   lag=i + 1, propertylabel='En')
...
def CalculateMoranAutoPolarizability(mol):
    ...
    for i in range(8):
        res['MATSp' + str(i + 1)] = _CalculateMoranAutocorrelation(mol,
                                                                   lag=i + 1, propertylabel='alapha')
...
if __name__ == '__main__':
    ...
    smi5 = ['COCCCC', 'CCC(C)CC', 'CC(C)CCC',
            'CC(C)C(C)C', 'CCOCCN', 'c1ccccc1N']
